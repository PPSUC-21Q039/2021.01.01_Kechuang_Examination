...
#       2. GPS数据对.xlsx（默认不输出），为程序处理后用于可视化分析事故多发区域的数据集，该数据集不做作业提交之用，仅用于提交至高德数据平台进行可视化分析。分析结果参见本目录下的：高德可视化.html 或访问： https://maplab.amap.com/share/mapv/ece53f4b48cc4d6c24faaedd2948f1fe

# 程序特点：
# 1. 程序注释完整清晰有条理，便于维护
# 2. 程序输入输出格式标准，与其他程序配合较多，避免了重复造轮子的行为
# 3. 程序对输入与输出文件做出了多次校验，避免文件不存在、文件损坏或文件在 Windows 平台下被锁定导致程序异常退出，并为进一步纠正问题提供了参照
# 4. 程序在开发过程中充分考虑了平台兼容性，并在多个平台（Windows 10，Windows 11，Debian，FreeBSD）下进行了实地测试，保证了程序的跨平台性（就当我说废话吧）
# 5. 程序对于网络结果极其谨慎，并且仔细考量了错误信息及处理方式
# 6. 程序输出完整、合理、直观，不仅便于读取，还便于后续查证及进一步处理，本程序的文件输出与第三题程序配合良好
# 7. 程序短小精干，逻辑严谨，注释完整易读
