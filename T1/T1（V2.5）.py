...
# 2. 程序对输入、输出的诸多文件进行了反复校验，避免了文件不存在、文件损坏或文件在 Windows 平台下被锁定导致程序异常退出的情况，且其退出时的错误警告能对解决实际运行中产生的问题提供较好的辅助作用
...
warnings.filterwarnings("ignore") # Ignore warnings of module: xlrd
